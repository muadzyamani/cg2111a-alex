...
ROBOT_CLAW_OPEN_WIDTH_MM = 320
ROBOT_CLAW_LENGTH_MM = 0.2*1000
...
def createLidarPlot():
    ...
    pointMarker = "circle"
    p.scatter(x='x', y='y', source=source, size=pointSize, color=pointColor, alpha=pointAlpha, 
    ...
    # Add a point 180mm in front of the robot (which faces "up" in the plot by default)
    point_distance = 185 # mm in front of robot
    astro_radius = 20

    point_x = 0
    point_y = point_distance
    p.circle(
        x=[point_x], 
        y=[point_y], 
        radius = astro_radius, 
        ...
        line_width=1,            # Outline thickness
        alpha=0.8,               # Slightly transparent
        legend_label="Target Point"
    )
    
        # Add onlyfans  rectangle 200mm infront the robot
    onlyfans_distance = 400 # mm infront robot
    onlyfans_width = 200     # mm width
    onlyfans_height = 40     # mm height
    
    # Calculate rectangle coordinates (centered behind the robot)
    onlyfans_x = onlyfans_width/1000/2  # centered on x-axis
    onlyfans_y = onlyfans_distance 
    
    p.rect(
        x=onlyfans_x,
        y=onlyfans_y,
        width=onlyfans_width,
        height=onlyfans_height,
        fill_color=None,
        line_color="blue",
        line_width=1,
        alpha=0.8,
        legend_label="onlyfans"
    )
    
       # Add claw closed rectangle  infront the robot
    closed_distance = 200
    closed_width = 200    # mm width
    closed_height = 74  # mm height


   # Calculate rectangle coordinates (centered infront the robot)
    closed_x = closed_width/1000/2  # centered on x-axis
    closed_y = closed_distance
    ...
    # Add open claw infront the robot
    claw_distance = point_distance - astro_radius/2 + 10 # mm infront robot
    claw_width = ROBOT_CLAW_OPEN_WIDTH_MM    # mm width
    claw_height = 40     # mm height
    
    # Calculate rectangle coordinates (centered behind the robot)
    rect_x = claw_width/1000/2  # centered on x-axis
    rect_y = claw_distance 
    ...
    p.rect(
        ...
        # Add distance measurement line (vertical)
    p.segment(
        x0=[0], y0=[closed_y + closed_height/2],          # Start at robot origin
        x1=[0], y1=[400],    # End at target point
        line_color="purple",
        line_width=1.5,
        line_dash="dashed",
        alpha=0.6
        ...
        # Add distance measurement line behind (vertical)
    p.segment(
        x0=[0], y0= [-(closed_y + closed_height/2)],          # Start at robot origin
        x1=[0], y1=[-400],    # 
        line_color="purple",
    ...
    # p.text(
    #     x=[10],                  # Small offset from line
    #     y=[point_y/2],           # Midpoint
    #     text=[f"{point_distance} mm"],
    #     text_color="purple",
    #     text_font_size="10pt",
    ...
    # Add dispenser rectangle 150mm behind the robot
    dispenser_distance = 110  # mm behind robot
    dispenser_width = 200     # mm width
    dispenser_height = 10     # mm height
    
    # Calculate rectangle coordinates (centered behind the robot)
    rect_x = -dispenser_width/1000/2  # centered on x-axis
    rect_y = -(dispenser_distance - dispenser_height/2)  # positioned behind
    
    p.rect(
        x=rect_x,
        y=rect_y,
        width=dispenser_width,
        height=dispenser_height,
        fill_color=None,
        line_color="green",
        line_width=1,
        alpha=0.8,
        #legend_label="Dispenser"
    )
    
        # Add a opening claw intersection trajectory in front of the robot
    intersection_distance = 185 # mm in front of robot

    intersection_x = 0
    intersection_y = claw_distance -  claw_height
    p.circle(
        x=[intersection_x], 
        y=[intersection_y], 
        radius = ROBOT_CLAW_OPEN_WIDTH_MM/2, 
        fill_color=None,          # No fill
        line_color="green",       # Outline color
        line_width=1,            # Outline thickness
        line_dash="dashed",

        alpha=0.8,               # Slightly transparent
        #legend_label="Tintersex"
    )
